...
{
    echo web_log.tsv|awk -F'\t' '{print $1}'|grep -E "^[0-9]"|sort|uniq -c|sort -nr|head -n 100;
} >> web_cal_data.txt
printf "统计最频繁被访问的URL TOP 100"> web_cal_data.txt
{
    echo web_log.tsv|awk -F'\t' '{print $5}'|sort|uniq -c|sort -nr|head -n 100;
}>> web_cal_data.txt
...
{
	echo web_log.tsv|awk -F'\t' '{if($6=="403")print $5,$6}'|sort|uniq -c|sort -nr|head -n 100;
} >> web_cal_data.txt
printf "404:">> web_cal_data.txt
{
	echo web_log.tsv|awk -F'\t' '{if($6=="404")print $5,$6}'|sort|uniq -c|sort -nr|head -n 100;
}>> web_cal_data.txt
...
if [ "$url" = "-h" ]||[ "$url" = "-help" ];then
	...
	{
		echo web_log.tsv|awk -F'\t' '{if("$5"=="'"$url"'")print "$1","$5"}'|sort|uniq -c|sort -nr|head -n 100;
	}>> web_cal_data.txt
fi
echo "处理完成"
