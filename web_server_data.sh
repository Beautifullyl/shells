...
#5.最后写入文件
{
    cmd web_log.tsv|awk -F'\t' '{print $1}'|sort|uniq -c|sort -nr|head -n 100
} >> web_cal_data.txt 
printf "统计访问来源主机TOP 100 IP和分别对应出现的总次数">> web_cal_data.txt
#grep使用正则表达式搜索文本，^表示以后面开头的元素，[]表示范围
{
    cmd web_log.tsv|awk -F'\t' '{print $1}'|grep -E "^[0-9]"|sort|uniq -c|sort -nr|head -n 100
} >> web_cal_data.txt
printf "统计最频繁被访问的URL TOP 100"> web_cal_data.txt
{
    cmd web_log.tsv|awk -F'\t' '{print $5}'|sort|uniq -c|sort -nr|head -n 100
}>> web_cal_data.txt
...
{
	cmd web_log.tsv|awk -F'\t' '{if($6=="403")print $5,$6}'|sort|uniq -c|sort -nr|head -n 100
} >> web_cal_data.txt
printf "404:">> web_cal_data.txt
{
	cmd web_log.tsv|awk -F'\t' '{if($6=="404")print $5,$6}'|sort|uniq -c|sort -nr|head -n 100 
}>> web_cal_data.txt
...
if [ "$url" = "-h" ]||[ "$url" = "-help" ];then
	...
	{
		cmd web_log.tsv|awk -F'\t' '{if("$5"=="'"$url"'")print "$1","$5"}'|sort|uniq -c|sort -nr|head -n 100 
	}>> web_cal_data.txt
fi
echo "处理完成"
